...
public class TaskQueryIntegrationTest {
        ...
        void setUp() {
                ...
                taskRepository.save(
                                new Task("Admin Task", "Secret", Status.PENDING, Priority.HIGH,
                                                LocalDateTime.now().plusDays(1),
                                                admin));
        }
        ...
        void shouldNotSeeDeletedTasksByDefault() throws Exception {
                mockMvc.perform(get("/api/v1/tasks"))
                                .andExpect(status().isOk())
                                .andExpect(jsonPath("$.content", hasSize(3))) // 1, 2, 3 (Admin Task hidden by
                                                                              // ownership)
        ...
        void adminShouldSeeAllTasks() throws Exception {
                mockMvc.perform(get("/api/v1/tasks"))
                                .andExpect(status().isOk())
                                .andExpect(jsonPath("$.totalElements").value(4)); // 1, 2, 3, Admin Task (Deleted still
                                                                                  // hidden by
                                                                                  // default)
        }
        ...
        void shouldSortByPriorityDesc() throws Exception {
                mockMvc.perform(get("/api/v1/tasks?sortBy=priority&direction=desc"))
                                .andExpect(status().isOk())
                                .andExpect(jsonPath("$.content[0].priority").value("MEDIUM")) // MEDIUM > LOW in string
                                                                                              // sort? Wait.
